...
class PodiumNN(nn.Module):
    ...
    def __init__(self, input_dim, num_classes):
        ...
        self.model = nn.Sequential(
            # Input layer
            nn.Linear(input_dim, 64),  # Reduced size for stability
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Dropout(0.2),
...
def main():
    try:
        ...
        model = PodiumNN(input_dim, num_classes).to(device)
        
        # Define loss function and optimizer
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=0.0001, weight_decay=1e-4)  # Lower learning rate and add weight decay
        ...
        print("\nPredicted vs Actual Podiums for Each Race (PyTorch Neural Network):")
        print("=" * 80)
        ...
        print("\n" + "=" * 80)
        print("\nCustom F1 Podium Prediction Metrics (PyTorch Neural Network):")
        ...
        }, 'basic_podium_nn_model.pth')
        
        print("\nModel saved to 'basic_podium_nn_model.pth'")
