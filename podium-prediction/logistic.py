...
def main():
    ...
    train_df = df[(df['year'] < 2024) & (df['year'] >= 2014)]
    ...
    # Create and train the logistic regression model with preprocessing
    model = Pipeline(steps=[
        ('preprocessor', preprocessor),
        ('classifier', LogisticRegression(max_iter=1000, random_state=42, solver='liblinear', multi_class='ovr', class_weight='balanced'))
    ])
    ...
    # Get the class indices for 'first', 'second', 'third'
    classes = model.classes_
    first_idx = np.where(classes == 'first')[0][0]
    second_idx = np.where(classes == 'second')[0][0]
    third_idx = np.where(classes == 'third')[0][0]
    ...
    print("\nPredicted vs Actual Podiums for Each Race:")
    print("=" * 80)
    ...
    for race_id, race_data in race_groups:
        ...
        if len(actual_podium) > 0:
            for _, row in actual_podium.iterrows():
                print(f"  {row['true_label'].capitalize()}: Driver {row['driverId']} (Constructor {row['constructorId']})")
        else:
    ...
    print("\n" + "=" * 80)
    print("\nCustom F1 Podium Prediction Metrics:")
